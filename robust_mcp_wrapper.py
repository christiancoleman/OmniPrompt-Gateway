#!/usr/bin/env python
"""
Robust MCP Wrapper - Handles npx issues and uses direct node execution
"""
import json
import subprocess
import sys
...
class RobustMCPClient:
	...
	def _add_server(self, name: str, command: List[str]):
		...
		print(f"Starting server with command: {' '.join(command[:3])}...")
	...
	def _send_request(self, server_name: str, method: str, params: Dict) -> Optional[Dict]:
		...
		try:
			...
			stderr = process.stderr.read() if process.stderr is not None else ""
			print(f"Timeout waiting for response. Stderr: {stderr}")
			return None
...
# Use the same integration functions as before
